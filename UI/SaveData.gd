...
func _save_data(pos, ipos, savefile, world, score):
	...
	if world == "OverWorld":
		if savefile.x > 0:
			...
			var up_task : FirestoreTask = firestore_collection.update(email, {'email': email, 'username': query.doc_fields.get("username"), 'score': score, 'position': pos2, 'insidePosition': ipos2, 'world': world})
		...
		if savefile.x > 0:
			...
			firestore_collection.get(email)
			var query : FirestoreDocument = yield(firestore_collection, "get_document")
			var up_task : FirestoreTask = firestore_collection.update(email, {'email': email, 'username': query.doc_fields.get("username"), 'score': score, 'position': ipos2, 'insidePosition': pos2, 'world': world})
			var document : FirestoreDocument = yield(up_task, "task_finished")
		elif savefile.y > 0:
			var firestore_collection : FirestoreCollection = Firebase.Firestore.collection("userdata1")
			firestore_collection.get(email)
